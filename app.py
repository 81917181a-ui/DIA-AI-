...
def call_gemini(model, history: list, user_message: str) -> str:
    # historyはst.session_state["messages"]形式（role/content）をGemini形式に変換
    gemini_history = []
    for m in history:
...
def main():
    ...
    if model is None:
        st.warning(
            "GEMINI_API_KEY が設定されていません。Render の環境変数に GEMINI_API_KEY を"
            "設定してください。"
        )
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            if model is None:
                reply = "GEMINI_API_KEY が未設定のため応答できません。"
                st.markdown(reply)
            else:
